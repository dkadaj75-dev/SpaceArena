...
describe("MatchmakingScreen", () => {
  it("moves searching -> found -> joining", () => {
    const screen = new MatchmakingScreen(document.body, configs(), {
      onCancel: vi.fn(),
      onBack: vi.fn(),
      onRetry: vi.fn(),
    });
  ...
  it("shows an honest retry action after queue recovery fails", () => {
    const onRetry = vi.fn();
    const screen = new MatchmakingScreen(document.body, configs(), {
      onCancel: vi.fn(),
      onBack: vi.fn(),
    ...
    screen.begin();
    screen.interrupted();
    expect(screen.state).toBe("interrupted");
    expect(document.body.textContent).toContain("Search interrupted — tap to retry");
    document.querySelector<HTMLButtonElement>('[data-matchmaking-action="retry"]')!.click();
